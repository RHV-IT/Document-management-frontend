...
const CLEARANCE_COLORS: Record<ClearanceLevel, { background: string; color: string; border: string }> = {
  public: {
    background: '#10b981',
    color: '#065f46',
    border: '#10b981',
...
const clearanceLevels: ClearanceLevel[] = ['public', 'internal', 'confidential', 'highly_confidential']
const clearanceLevelOrder: Record<ClearanceLevel, number> = {
  public: 0,
  internal: 1,
  confidential: 2,
...
const isClearanceLevel = (level: string): level is ClearanceLevel => clearanceLevels.includes(level as ClearanceLevel)

const getHighestClearanceLevel = (levels: ClearanceLevel[]): ClearanceLevel | null => {
  if (levels.length === 0) return null
  return levels.reduce((highest, level) => {
    return clearanceLevelOrder[level] > clearanceLevelOrder[highest] ? level : highest
  })
}

const getUserHighestClearanceLevel = (user: any): ClearanceLevel | null => {
  // If we have profiles, collect all confidentiality levels from all profiles
  if (user.profiles && user.profiles.length > 0) {
    const allLevels: ClearanceLevel[] = []
    user.profiles.forEach((profile: any) => {
      if (profile.confidentialityLevels && Array.isArray(profile.confidentialityLevels)) {
        profile.confidentialityLevels.forEach((level: unknown) => {
          if (typeof level === 'string' && isClearanceLevel(level) && !allLevels.includes(level)) {
    ...
    if (allLevels.length > 0) {
      return getHighestClearanceLevel(allLevels)
    }
  }
  // Fallback to legacy fields
  if (user.confidentialityLevels && Array.isArray(user.confidentialityLevels) && user.confidentialityLevels.length > 0) {
    const validLevels = user.confidentialityLevels.filter(isClearanceLevel) as ClearanceLevel[]
    if (validLevels.length > 0) {
      return getHighestClearanceLevel(validLevels)
    }
  }
  if (user.confidentialityLevel && isClearanceLevel(user.confidentialityLevel)) {
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        {(() => {
                          const lvl = user ? getUserHighestClearanceLevel(user) : ''
                          return lvl ? (
                            <Badge
                              className="text-xs"
                              style={{
